...
namespace NordVpn.Map
{
	public class MapView : UserControl, IComponentConnector
	{
		...
		private IAnalyticsManager _appEventManager;

		private bool _sendDragAppEvent;

		private bool _sendZoomAppEventOnMouseWheelScroll;

		private bool _sendZoomAppEventOnButtonsClick;
		...
		public MapView()
		{
			...
			base.Loaded += new RoutedEventHandler(this.Map_Loaded);
		}
		...
		private void StopPan(object sender, EventArgs e)
		{
			...
			if (this._sendDragAppEvent)
			{
				this._appEventManager.TriggerEvent("Map", "Drag");
				this._sendDragAppEvent = false;
			}
		}
		...
		private void ZoomDelta(int delta)
		{
			...
			if (this._sendZoomAppEventOnButtonsClick)
			{
				this._appEventManager.TriggerEvent("Map", "ZoomUsingButtons");
				this._sendZoomAppEventOnButtonsClick = false;
			}
		}
		...
		private void ZoomMapInteraction(object sender, MouseWheelEventArgs e)
		{
			...
			if (this._sendZoomAppEventOnMouseWheelScroll)
			{
				this._appEventManager.TriggerEvent("Map", "ZoomUsingWheel");
				this._sendZoomAppEventOnMouseWheelScroll = false;
			}
			if (!e.Handled)
			{
				e.Handled = true;
