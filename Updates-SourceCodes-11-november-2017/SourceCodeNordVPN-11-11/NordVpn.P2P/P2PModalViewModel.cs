...
namespace NordVpn.P2P
{
	public sealed class P2PModalViewModel : AppScreen
	{
		...
		public GeographicalServer BestP2PServer
		{
			...
			set
			{
				this.Set<GeographicalServer>(ref this._bestP2PServer, value, "BestP2PServer");
			}
		}
		...
		public P2PModalViewModel(EventFacade eventFacade, IBrowser browser, BestUserServerProvider serverProvider, BestVpnServerConnector vpnConnector)
		{
			...
			this._serverProvider.ServersChanged += new EventHandler(this.OnServersChanged);
		...
		public void OpenServerList()
		{
			...
			this._eventFacade.Publish(new NavigateToCategoryServersGroupMessage());
			this._eventFacade.Publish(new NavigateToServersDetailsMessage<Category>(p2PServerCategory));
			this.TryClose(null);
		}

		public void OpenArticle()
		{
			this._browser.Open("https://support.nordme.org/hc/en-us/articles/206939499-Peer-to-Peer-traffic-with-NordVPN");
		}
